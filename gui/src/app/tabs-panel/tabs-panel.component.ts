import { Component, OnInit, ContentChildren, QueryList, Input, TemplateRef, AfterContentInit } from '@angular/core';
...
export class TabsPanelComponent implements AfterContentInit {
  ...
  ngAfterContentInit() {
    ...
    selectedTabs.length > 1 && selectedTabs.forEach((tab, i) => (tab.selected = (i === 0)));
    !selectedTabs.length && this.tabs.length && (this.tabs.first.selected = true);
  }
